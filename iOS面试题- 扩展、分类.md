...
分类不能直接添加属性的根本原因在于**编译机制和内存布局的限制**：

1. **分开编译导致内存布局提前确定**
   - 分类和主类是分开编译的独立文件
   - 主类在编译时就已经确定了内存布局（包括所有实例变量的偏移量）
   - 分类在运行时才动态附加到类上，此时内存布局已经固定，无法修改

2. **属性 = 实例变量 + getter/setter 方法**
   - 属性本质上需要实例变量来存储数据
   - 分类不能添加实例变量（没有存储空间）
   - 即使可以添加 getter/setter 方法，也没有实例变量来存储和读取数据

3. **编译器的限制**
   - 编译器在编译主类时，无法预知将来会有哪些分类
   - 即使分类先编译，编译器也无法修改已经确定的主类内存布局
   - 如果允许多个分类添加实例变量，会导致内存布局不确定，无法计算正确的偏移量
...
分类分开编译
    ↓
主类在编译时确定内存布局（包括所有实例变量）
    ↓
分类运行时才附加，无法修改已确定的内存布局
    ↓
分类不能添加实例变量（没有存储空间）
    ↓
...
没有实例变量 → 无法直接添加属性
    ↓
解决方案：关联对象
    - 可以添加 getter/setter 方法（分类支持添加方法）
    - 使用关联对象在对象外部存储数据（解决存储问题）
    - 实现了类似属性的功能
```

#### 6.2.3 关键点澄清

- ✅ **分类可以添加方法**：包括 getter/setter 方法
- ❌ **分类不能添加实例变量**：没有存储空间
- ⚠️ **问题所在**：可以写 getter/setter 方法，但没有实例变量来存储数据
- ✅ **关联对象的作用**：提供外部存储空间，让 getter/setter 方法可以正常工作
